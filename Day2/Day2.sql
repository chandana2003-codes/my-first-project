Day2 folder lo Day2.sql lo idi paste chey:sqlSELECT * FROM COLLEGE.MITS.STUDENT;
SELECT * FROM BANK.HDFC.EMPLOYEE;
SELECT * FROM CUSTOMER;

ALTER TABLE CUSTOMER ADD COLUMN PHONE INT, SALARY FLOAT;

DESCRIBE TABLE CUSTOMER;

ALTER TABLE CUSTOMER ADD COLUMN COMMENTS STRING;
ALTER TABLE CUSTOMER DROP COLUMN PHONE;
ALTER TABLE CUSTOMER DROP COLUMN EMAIL;

UPDATE CUSTOMER 
    SET SALARY = 50000, COMMENTS = 'SR '
WHERE CID = 4;

SELECT * FROM CUSTOMER;

UPDATE customer
SET salary = 60000, comments = 'High value customer' 
WHERE cid IN (1,2,3,7,8);

UPDATE customer
SET salary = 0, comments = 'LOW value customer';

CREATE DATABASE HR;
CREATE SCHEMA HR.VIT;
CREATE TABLE HR.VIT.employees (
...
SELECT
  ...
  COUNT(employee_id) AS num_employees_joined
FROM employees
GROUP BY hire_year
ORDER BY hire_year DESC NULLS LAST;

SELECT * FROM EMPLOYEES;
SELECT 'EMPLOYEE - ', COUNT(*) FROM EMPLOYEES;
SELECT 'aNIL' || ' kUMAR' AS NAME;
SELECT 500 + 650 + 730;